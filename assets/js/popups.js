...
const EDC_POPUPS = (() => {
  ...
  function renderBar(c) {
    ...
    if (c.cta_label && c.cta_url) { const a = el("a", "edc-popup-cta", esc(c.cta_label)); a.href = c.cta_url; bar.appendChild(a); }
    bar.appendChild(closeBtn(bar));
    document.body.prepend(bar); markSeen(c);
  }
  function renderBanner(c) {
    ...
    banner.innerHTML = `<div class="edc-popup-banner-inner">
      ...
      <div class="edc-popup-banner-body">
        ...
        ${c.cta_label && c.cta_url ? `<a class="edc-popup-cta" href__="${esc(c.cta_url)}">${esc(c.cta_label)}</a>` : ""}
      </div></div>`;
    banner.appendChild(closeBtn(banner));
    document.body.appendChild(banner); markSeen(c);
  }
